...
# set the maximum number of frames for the movie
max_pics = 40
...
start_pic = pygame.image.load(os.path.join('/home/pi/animation', 'Opening_Screen.jpg'))
start_pic_fix = pygame.transform.scale(start_pic, (width, height))
video_pic = pygame.image.load(os.path.join('/home/pi/animation', 'Video_Processing_Screen.jpg'))
...
def take_pic():
    ...
    pics_taken += 1
    camera.capture(os.path.join('/home/pi/animation', 'image_' + str(pics_taken) + '.jpg'), use_video_port = True)
    prev_pic = pygame.image.load(os.path.join('/home/pi/animation', 'image_' + str(pics_taken) + '.jpg'))
...
def animate():
    ...
    for pic in range(1, pics_taken):
        anim = pygame.image.load(os.path.join('/home/pi/animation', 'image_' + str(pic) + '.jpg'))
...
def make_movie():
    ...
    os.system("avconv -r " + str(fps) + " -i " + str((os.path.join('/home/pi/animation', 'image_%d.jpg'))) + " -vcodec libx264 video.mp4")
    play_movie()
    sys.exit(0)

def play_movie():
    os.system("omxplayer video.mp4")
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    camera.start_preview()
                    intro = False
        screen.blit(start_pic_fix_custom, (0, 0))
        pygame.display.update()
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if (event.key == pygame.K_ESCAPE):
                    quit_app()
                elif event.key == pygame.K_SPACE: # take picture and preview
                    take_pic()
